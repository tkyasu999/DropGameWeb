...
DropGameWeb は、Python と pygame で実装されたスイカ（Suika）ゲームのようなボールが落ちるパズルゲームです。同じ種類のフルーツを重ねて合体させ、スコアを競います。
...
## 遊び方
- マウスでドロップ位置を調整し、クリックでフルーツを落とす。
- 同じレベルのフルーツが接触すると合体して次のレベルに進化。
- フルーツが上部の赤いラインを超えるとゲームオーバー。
- Rキーでリセット。
